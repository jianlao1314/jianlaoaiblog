...
在线演示 ➡️ [https://ai.5334427.xyz](https://ai.5334427.xyz)

官方文档 ➡️ [vhAstro-Theme](https://ai.5334427.xyz/article/astro-theme-vhastro-theme)
...
- [x] 高效的性能优化
- [x] 优秀的开发体验
- 🎨 现代化设计：采用 Material Design 3 设计语言，支持亮色/暗色主题
- 📱 响应式布局：完美适配桌面端和移动端
- 🔍 强大的搜索功能：支持标题、标签、分类和内容搜索
- 🏷️ 标签系统：支持多标签筛选和标签云展示
- 📊 数据统计：文章阅读量、点赞数、评论数等数据统计
- 💬 评论系统：支持 Giscus 评论系统
- 🔄 自动部署：支持 Vercel、Netlify 等平台自动部署
- 📈 SEO 优化：支持多搜索引擎 URL 推送，包括百度、Google、Bing、360搜索等
- 🚀 性能优化：支持 Cloudflare Workers KV 存储，避免重复推送 URL
...
export default {
	...
	data: [
		{
			name: "建佬AI博客",
			link: "https://ai.5334427.xyz",
			avatar: "https://q1.qlogo.cn/g?b=qq&nk=1655466387&s=640",
			descr: "运气是计划之外的东西."
		},
		{
			name: "建佬AIAPI",
			link: "https://api.5334427.xyz",
			avatar: "https://api.5334427.xyz/static/images/logo.webp",
	...
	data: [
		{
			...
			auther: "建佬AI博客",
			date: "2025-03-06",
			link: "https://ai.5334427.xyz/article/Lenis-in-Astro",
		...
		},
		{
			...
			auther: "建佬AI博客",
			date: "2025-03-03",
			link: "https://ai.5334427.xyz/article/astro-twikoo",
		...
		},
		{
			...
			auther: "建佬AI博客",
			date: "2025-03-02",
			link: "https://ai.5334427.xyz/article/astro-theme-vhastro-theme",
...
top: false # 是否置顶文章
hide: false # 是否隐藏文章
needVerification: true  # 这篇文章需要验证
<!-- 页面独有 -->
type: "links" # 页面类型
...
export default {
  ...
  Description: '建佬AI博客 专注于前开发与相关技术的实战分享，涵盖Vue框架、Node.js、Serverless等，并涉及Node、Python、Linux、Docker等领域。同时，博客也分享作者的生活、音乐和旅行的热爱。',
  Author: '.𝙃𝙖𝙣',
  Motto: '运气是计划之外的东西.',
  Avatar: 'https://q1.qlogo.cn/g?b=qq&nk=1655466387&s=640',
  // Cover 网站缩略图
  ...
  Navs: [
    ...
    { text: 'API', link: 'https://api.5334427.xyz/', target: true, icon: 'Nav_link' },
  ],
  // 侧边栏个人网站 - 暂时关闭
  WebSites: [
    // 仅支持 SVG 且 SVG 需放在 public/assets/images/svg/ 目录下，填入文件名即可（封装了 SVG 组件 为了极致压缩 SVG）
    ...
    { text: 'HanAnalytics', link: 'https://analytics.5334427.xyz', icon: 'WebSite_analytics' },
  ],
  // 侧边栏展示
  AsideShow: {
    // 是否展示个人网站
  ...
  HanAnalytics: { enable: true, server: 'https://analytics.5334427.xyz', siteId: 'Hello-HanHexoBlog' },
  // Google 广告
  GoogleAds: {
...
本项目支持多搜索引擎 URL 推送功能，可以自动将新发布的文章 URL 推送到各大搜索引擎，提高网站收录效率。
...
1. 在 `config.ts` 中配置搜索引擎参数：

```typescript
export const SeoPush = {
  enable: true,
  engines: {
    baidu: {
      enable: true,
...
2. 配置 Cloudflare Workers KV：

```bash
# 创建 KV 命名空间
wrangler kv:namespace create "PUSHED_URLS"

# 将生成的命名空间 ID 替换到 wrangler.toml 中
```
...
```toml
kv_namespaces = [
  { binding = "PUSHED_URLS", id = "your-kv-namespace-id" }
]
```
...
- 支持多个主流搜索引擎
- 使用 Cloudflare Workers KV 存储避免重复推送
- 自动检测新文章并推送
- 支持自定义推送参数
- 提供详细的推送状态日志
...
1. 需要在各个搜索引擎的站长平台获取相应的 token
2. 建议在正式环境中启用此功能
3. 确保 Cloudflare Workers KV 配置正确
4. 定期检查推送状态和收录情况
